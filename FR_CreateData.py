import cv2
import numpy as np
import time

# 얼국 인식용 xml 파일
# OpenCV에서 기본 라이브러리로 제공하는 Haar Cascade는 기계학습을 이용한 객체 탐지 알고리즘입니다.
# CNN이나 RSTM 등의 신경망이 아닌, 하르 특징 (Haar Features) 계산을 통해 객체를 찾습니다.
face_classifier = cv2.CascadeClassifier('haarcascade_frontalface_default.xml')

# 캠 크기가 640, 480

# 전체 사진에서 얼굴 부위만 잘라 리턴
def face_extractor(img):
    # 흑백처리
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    # 얼굴 찾기
    faces = face_classifier.detectMultiScale(gray, 1.3, 5)
    # 찾은 얼굴이 없으면 None으로 리턴
    if faces is ():
        return None
    # 얼굴들이 있으면
    for (x, y, w, h) in faces:
        b_center = (x+w/2, y+h/2)
        print(b_center)
        if b_center[0] < 280 or b_center[0] > 360 or b_center[1] < 200 or b_center[1] > 280:
            return None
        # 해당 얼굴 크기만큼 cropped_face에 잘라 넣기
        # 근데... 얼굴이 2개 이상 감지되면??
        # 가장 마지막의 얼굴만 남을 듯
        cropped_face = img[y:y + h, x:x + w]
    # cropped_face 리턴
    return cropped_face, b_center


# 카메라 실행
cap = cv2.VideoCapture(0)
# 저장할 이미지 카운트 변수
start = time.time()
count = 0
while True:
    if time.time()-start > 5:
        break
    # 카메라로 부터 사진 1장 얻기
    ret, frame = cap.read()
    # 얼굴 감지 하여 얼굴만 가져오기
    result = face_extractor(frame)
    if result is not None:
        face = result[0]
        b_center = result[1]
        # cv2.line(frame, (int(b_center[0]), int(b_center[1])), (int(b_center[0]), int(b_center[1])), (0, 0, 255), 3)
        count += 1
        # 얼굴 이미지 크기를 200x200으로 조정
        face = cv2.resize(face, (200, 200))
        # 조정된 이미지를 흑백으로 변환
        # face = cv2.cvtColor(face, cv2.COLOR_BGR2GRAY)
        # faces폴더에 jpg파일로 저장
        # ex > faces/user0.jpg   faces/user1.jpg ....
        file_name_path = 'faces/seong_bin/user' + str(count) + '.jpg'
        cv2.imwrite(file_name_path, face)

    # 화면에 얼굴과 현재 저장 개수 표시
    cv2.putText(frame, str(count), (50, 50), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 255, 0), 2)
    cv2.rectangle(frame, (230, 150), (410, 330), (255, 0, 0), 2)
    cv2.imshow('Face Cropper', frame)

    if cv2.waitKey(1) == 13 or cv2.waitKey(1) == 27 or count == 200:
        break


cap.release()
cv2.destroyAllWindows()
print('Colleting Samples Complete!!!')